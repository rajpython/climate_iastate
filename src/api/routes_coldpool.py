...
def get_cold_pool_survey_replicate(
    ...
    annual_p = MODEL_DIR / f"survey_replicate_annual_{source}.parquet"
    hauls_p = MODEL_DIR / f"survey_replicate_{source}.parquet"
    if not annual_p.exists():
        raise HTTPException(
            status_code=503,
            detail=f"Survey replicate not built for {source!r}. Run: mhw-build-survey-replicate --source {source}",
        )
